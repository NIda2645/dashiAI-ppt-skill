# Imported Deck Fidelity Notes

This file tracks source decks imported into the component system and the design details that may be weakened when the original HTML is rebuilt as layout-specific React components.
...
| `/Users/jadon7/Downloads/claude design 测试/style2` | 5 | Imported as `style2_02`-`style2_06` |
...
Known import differences:

- Original used exact 1920x1080 pixel values and dense inline positioning; the imported version uses responsive `vw/vh`, so a few geometric proportions are approximate.
- Original had a large embedded custom Chinese display font; imported components use the current layout font roles, so the title character shape differs.
...
- Original table and chart spacing was tuned per slide; imported components normalize table, bar, card, and footer spacing to shared `rp-*` CSS variables.
- Original fixed brand palette is preserved as `--rp-*` local CSS variables.
...
- `xhs16`-`xhs18`: 词元概率 and capability section
...
- Original arrow/arc decorative shapes on the cover are simplified to a reusable arc and text composition.
...
- Original palette is preserved as local XHS CSS variables (`--xhs-*`).
...
- `xhs2_16`-`xhs2_19`: 词元概率, closed system, probabilistic output, and verification cases
...
- Original uses six accent colors with per-slide manual emphasis; imported pages keep these as local CSS variables and a single active accent per slide.
...
- Original line breaks and 1920px typography are approximated with responsive CSS, so some large statement pages wrap differently.
...
- `xhs3_10`-`xhs3_18`: model collapse, SLOP, originality, expertise, Agent/context, 词元预测, tests, and shadow/source conclusion
...
- Original `hfx02` wires a live Rive runtime and 13 MB `.riv`/data files. The imported page keeps the layout and game-card affordance but replaces the live game with CSS orbital visuals.
- Source pages used exact 1920x1080 pixel CSS and embedded subset fonts; imported components use current layout font roles and responsive CSS, so exact letterform, line break, and object placement will differ.
...
- Original uses embedded SVG symbols and exact 1920x1080 CSS; imported pages redraw mascot/logo primitives inline and map typography to current layout font roles.
...
- Original has Google font choices for Chinese serif/sans and Instrument Serif; imported pages approximate them with current title/body/number/mono font roles.
- Original chip shadows, aura blur, and per-row offsets are preserved directionally but normalized into prefixed `st1-*` CSS.

## `style2` -> `style2_02`-`style2_06`
...
- Original uses Noto Serif TC, Noto Sans TC, and JetBrains Mono; imported pages map these roles to current layout font roles.
